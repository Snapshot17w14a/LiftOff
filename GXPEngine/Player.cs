    ...
    internal class Player : Sprite
    {
        ...
        private void TEMPSpawnEnemy()
        {
            if(_spawnTimer >= _dataStorage.EnemySpawnInterval * 1000)
            {
                ...
                Enemy enemy = new Enemy((new Vector2(x, y) - _dataStorage.TargetVectors[index]).Normalize());
                enemy.SetXY(_dataStorage.TargetVectors[index].x, _dataStorage.TargetVectors[index].y);
            }
        }
